...
# (ACTIVE @profile, RUN THE COMMAND "mpirun -n 4 kernprof -v -l 2d_gather_scatter_func_profiler_mul.py" and "python3 -m line_profiler 2d_gather_scatter_func_profiler_mul.py.lprof > results.txt" in cmd)
@profile
def main():
    # INPUT PARAMETERS ---------------------------------
    nelx = 200
    ...
    while loop < 2000:
        ...
        if MPI.COMM_WORLD.rank == 1:
            MPI.COMM_WORLD.send(iter_t1, dest=0, tag=11)
            MPI.COMM_WORLD.send(before_cur_pros_usage1, dest=0, tag=13)
            MPI.COMM_WORLD.send(after_cur_pros_usage1, dest=0, tag=15)
            MPI.COMM_WORLD.send(iter_t2, dest=0, tag=16)
            MPI.COMM_WORLD.send(before_cur_pros_usage2, dest=0, tag=18)
            MPI.COMM_WORLD.send(after_cur_pros_usage2, dest=0, tag=20)
        elif MPI.COMM_WORLD.rank == 2:
            MPI.COMM_WORLD.send(iter_t1, dest=0, tag=21)
            MPI.COMM_WORLD.send(before_cur_pros_usage1, dest=0, tag=23)
            MPI.COMM_WORLD.send(after_cur_pros_usage1, dest=0, tag=25)
            MPI.COMM_WORLD.send(iter_t2, dest=0, tag=26)
            MPI.COMM_WORLD.send(before_cur_pros_usage2, dest=0, tag=28)
            MPI.COMM_WORLD.send(after_cur_pros_usage2, dest=0, tag=30)
        elif MPI.COMM_WORLD.rank == 3:
            MPI.COMM_WORLD.send(iter_t1, dest=0, tag=31)
            MPI.COMM_WORLD.send(before_cur_pros_usage1, dest=0, tag=33)
            MPI.COMM_WORLD.send(after_cur_pros_usage1, dest=0, tag=35)
            MPI.COMM_WORLD.send(iter_t2, dest=0, tag=36)
            MPI.COMM_WORLD.send(before_cur_pros_usage2, dest=0, tag=38)
            MPI.COMM_WORLD.send(after_cur_pros_usage2, dest=0, tag=40)
            ...
            iter_t1_2 = MPI.COMM_WORLD.recv(source=2, tag=21)
            before_cur_pros_usage1_2 = MPI.COMM_WORLD.recv(source=2, tag=23)
            after_cur_pros_usage1_2 = MPI.COMM_WORLD.recv(source=2, tag=25)
            iter_t2_2 = MPI.COMM_WORLD.recv(source=2, tag=26)
            before_cur_pros_usage2_2 = MPI.COMM_WORLD.recv(source=2, tag=28)
            after_cur_pros_usage2_2 = MPI.COMM_WORLD.recv(source=2, tag=30)
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            sheet1.cell(row=loop + 1, column=14).value = iter_t1_2
            sheet1.cell(row=loop + 1, column=15).value = before_cur_pros_usage1_2
            sheet1.cell(row=loop + 1, column=16).value = after_cur_pros_usage1_2
            sheet1.cell(row=loop + 1, column=17).value = after_cur_pros_usage1_2 - before_cur_pros_usage1_2
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            sheet2.cell(row=loop + 1, column=14).value = iter_t2_2
            sheet2.cell(row=loop + 1, column=15).value = before_cur_pros_usage2_2
            sheet2.cell(row=loop + 1, column=16).value = after_cur_pros_usage2_2
            sheet2.cell(row=loop + 1, column=17).value = after_cur_pros_usage2_2 - before_cur_pros_usage2_2

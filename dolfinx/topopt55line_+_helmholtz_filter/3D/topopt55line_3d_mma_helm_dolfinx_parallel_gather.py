...
def main(nelx, nely, nelz, volfrac, penal, rmin, ft, opt_solv):
    __time = time.time() ## hayoung
    if MPI.COMM_WORLD.rank == 0:
        print("Minimize compliance optimisation algorithm using HF")
        print("sizes: {0} X {1} X {2}, parallel core number is {3}".format(nelx, nely, nelz, MPI.COMM_WORLD.size))
    ...
    if MPI.COMM_WORLD.size > 1:
        if MPI.COMM_WORLD.rank == 1:
            msh_con = mesh.create_box(MPI.COMM_SELF, np.array([[0.0, 0.0, 0.0], [nelx, nely, nelz]]), [nelx, nely, nelz], cell_type=mesh.CellType.hexahedron, ghost_mode=mesh.GhostMode.shared_facet)
    ...
    print(f"mesh generation on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    ...
    print(f"function space defined on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    if MPI.COMM_WORLD.size == 1:
    ...
    print(f"mpi_serial_dof func on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    ...
    bcs = [bc_l]
    print(f"support defined on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    # DEFINE LOAD ---------------------------------
    ...
    print(f"load defined on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    ...
    # problem = fem.petsc.LinearProblem(k, f, bcs=bcs, petsc_options={"ksp_type": "preonly", "pc_type": "cholesky"})
    # problem = fem.petsc.LinearProblem(k, f, bcs=bcs, petsc_options={"ksp_type": "gmres"})
    # problem = fem.petsc.LinearProblem(k, f, bcs=bcs)
    problem = fem.petsc.LinearProblem(k, f, bcs=bcs, petsc_options={"ksp_type": "cg", "pc_type": "gamg", "ksp_monitor_singular_value": ""})
    print(f"problem defined on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    __time == time.time()
    ...
    print(f"ready to optimize on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
    while change > flag and loop < 2: # 000:
        loop += 1
        density_old.vector.array = density.vector.array
        # FE-ANALYSIS ---------------------------------
        __time == time.time()
        u_sol = problem.solve()
        print(f"linear solve finished on pros.{MPI.COMM_WORLD.rank}: {time.time() - __time}") ## hayoung
        # OBJECTIVE FUNCTION ---------------------------------
        ...
        # density_new = np.zeros(num_cells, dtype=np.float64)
        if opt_solv == 0:
            if MPI.COMM_WORLD.size == 1:
                ...
                while l2 - l1 > 1e-4:
                    ...
                    # print(np.linalg.norm(density_new))
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>6: rmin     = float(sys.argv[6])
    if len(sys.argv)>7: ft       = int(sys.argv[7])
    if len(sys.argv)>8: opt_solv = int(sys.argv[8])
    main(nelx, nely, nelz, volfrac, penal, rmin, ft, opt_solv)